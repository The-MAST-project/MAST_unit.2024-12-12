...
def expand_input(entry, allow_list=True):
    ...
    if path.is_dir():
        frames = sorted(p for p in path.iterdir() if p.is_file() and p.suffix.lower() in IMAGE_SUFFIXES)
        if not frames:
            msg = (
                f"directory {path} contains no image files (looked for "
    ...
    if not allow_list:
        msg = f"{path} is not a TIFF, a FITS file or a directory, and list files cannot nest"
        raise ValueError(msg)
...
def frame_stats(data, verbose=False):
    ...
    if std < DIGITIZATION_SIGMA:
        ...
        if verbose and std < 0.9 * DIGITIZATION_SIGMA:
            print(
                f"[stats] sigma-clipped scatter is {std:.3f} counts/px, below the "
...
def detect_segment(
    ...
    level,
    std,
    fwhm_guess=DEFAULT_FWHM_GUESS,
    nsigma=DEFAULT_NSIGMA,
    npixels=DEFAULT_NPIXELS,
    deblend=True,
    reject_spikes=True,
    verbose=False,
):
    ...
    if reject_spikes:
        ...
        if spikes.any():
            ...
            if verbose:
                where = ", ".join(
                    f"({float(catalog.x_centroid[i]):.0f}, {float(catalog.y_centroid[i]):.0f})"
                    for i in np.flatnonzero(spikes)
                )
                print(
                    f"[detect] ignored {int(spikes.sum())} single-pixel spike(s) "
    ...
    position = (float(catalog.x_centroid[brightest]), float(catalog.y_centroid[brightest]))
...
def detect_dao(data, level, std, fwhm_guess=DEFAULT_FWHM_GUESS, nsigma=DEFAULT_NSIGMA):
...
def detect_peak(data, level, std, fwhm_guess=DEFAULT_FWHM_GUESS, nsigma=DEFAULT_NSIGMA):
...
def locate_source(
    ...
    if found is not None:
        ...
        if verbose and n_found > 1:
            print(f"[detect] {n_found} {noun} found; using the brightest at ({xpeak:.1f}, {ypeak:.1f})")
    else:
        ...
        print(f"[detect] the {method} method found nothing; fell back to a smoothed peak search")
...
def measure_fwhm(data, position, background, fwhm_guess=DEFAULT_FWHM_GUESS):
    ...
    if at_source == 0:
        print("[fwhm] the centroid does not sit inside the half-maximum footprint; using --fwhm-guess instead")
        return fwhm_guess

    footprint = labels == at_source
    if footprint[0, :].any() or footprint[-1, :].any() or footprint[:, 0].any() or footprint[:, -1].any():
        print(
            "[fwhm] the half-maximum footprint reaches the edge of the search box; "
...
def measure_counts(data, position, radius, bkg_level, bkg_std, n_bkg_pix, gain=GAIN_E_PER_ADU, read_noise_e=READ_NOISE_E):
...
def make_figure(data, position, aperture, fwhm, bkg_level, out_path=None, annulus=None, show=True, close=True):
    ...
    ax_img.imshow(cutout.data, origin="lower", cmap="viridis", vmin=vmin, vmax=vmax, extent=extent)
    CircularAperture((0.0, 0.0), r=radius).plot(ax=ax_img, color="white", lw=1.5, label=f"aperture r = {radius:.1f} px")
    CircularAperture((0.0, 0.0), r=hwhm).plot(ax=ax_img, color="deepskyblue", lw=1.2, label=f"HWHM r = {hwhm:.1f} px")
    if annulus is not None:
...
def update_csv(csv_path, rows):
    ...
    kept = [row for i, row in enumerate(combined) if last_index[key(row.get("filename", ""))] == i]

    tmp_path = csv_path.with_suffix(csv_path.suffix + ".tmp")
    with tmp_path.open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=CSV_COLUMNS, restval="", extrasaction="ignore")
        writer.writeheader()
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--fwhm-guess", type=float, default=DEFAULT_FWHM_GUESS, help="approximate source FWHM in pixels, used for detection"
    )
    parser.add_argument("--radius", type=float, default=DEFAULT_RADIUS, help="fixed aperture radius in pixels")
    parser.add_argument(
        "--k-fwhm",
        type=float,
        default=None,
    ...
    parser.add_argument("--nsigma", type=float, default=DEFAULT_NSIGMA, help="detection threshold in background sigma")
    parser.add_argument(
        "--detect-method",
        choices=DETECT_METHODS,
    ...
    parser.add_argument(
        "--npixels",
        type=int,
        default=DEFAULT_NPIXELS,
        help="smallest connected area in pixels a segmentation detection may have (--detect-method segment only)",
    )
    parser.add_argument(
        "--no-deblend",
        action="store_true",
    ...
    parser.add_argument(
        "--annulus-bkg",
        action="store_true",
        help="estimate the background from a local annulus instead of the sigma-clipped whole frame",
    )
    parser.add_argument(
        "--r-in", type=float, default=ANNULUS_IN_FWHM, help="inner annulus radius in units of the measured FWHM"
    )
    parser.add_argument(
        "--r-out", type=float, default=ANNULUS_OUT_FWHM, help="outer annulus radius in units of the measured FWHM"
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        "--quiet",
        action="store_true",
        help="do not print the per-frame summary or the batch progress lines. Warnings and errors are still printed",
    )
    parser.add_argument("--gain", type=float, default=GAIN_E_PER_ADU, help="detector gain in e-/ADU")
    parser.add_argument("--read-noise", type=float, default=READ_NOISE_E, help="RMS read noise in e-")
    parser.add_argument("--plane", type=int, default=0, help="plane to use from a multi-plane TIFF or FITS stack")
    parser.add_argument(
        "--outfile",
        type=Path,
    ...
    parser.add_argument("--no-csv", action="store_true", help="do not write results to the CSV table")
...
def measure_single_image(
    ...
    position=None,
    nsigma=DEFAULT_NSIGMA,
    detect_method=DEFAULT_DETECT_METHOD,
    npixels=DEFAULT_NPIXELS,
    deblend=True,
    reject_spikes=True,
    annulus_bkg=False,
    r_in=ANNULUS_IN_FWHM,
    r_out=ANNULUS_OUT_FWHM,
    gain=GAIN_E_PER_ADU,
    ...
    if position is None:
        ...
        if verbose:
            print(f"[detect] measuring at the supplied centroid ({position[0]:.1f}, {position[1]:.1f}); detection skipped")
    ...
    if annulus_bkg:
        ...
        bkg_level, bkg_std, n_bkg_pix = estimate_background_annulus(data, position, ann_in, ann_out)
    ...
    if result["n_saturated"] > 0:
        print(
            f"WARNING: {result['n_saturated']} saturated pixels "
...
def report_measurement(result):
    ...
    if result["name"] is not None:
        print(f"\nfile        : {result['name']}")
    print(f"centroid    : ({result['x']:.2f}, {result['y']:.2f}) px [{result['detect_method']}]")
    print(f"FWHM        : {result['fwhm_pix']:.2f} px")
    print(f"aperture    : r = {result['radius_pix']:.2f} px  (area {result['area']:.1f} px^2)")
    print(
        f"background  : {result['bkg_level']:.3f} +/- {result['bkg_std']:.3f} "
        f"counts/px [{result['bkg_method']}, {result['n_bkg_pix']} px]"
    )
    print(f"counts      : {result['net_counts']:.4e} +/- {result['counts_err']:.2e} (SNR {result['snr']:.1f})")


def measure_images(
    images, plane=0, plot=False, verbose=False, print_output=False, on_error="skip", reuse_centroid=True, **params
):
    ...
    **params
    ...
    if isinstance(images, (str, Path)) or (isinstance(images, np.ndarray) and images.ndim == 2):
        images = [images]
    ...
    for index, image in enumerate(images):
        ...
        try:
            data = load_image(image, plane=plane, verbose=verbose) if is_path else image
            ...
            result = measure_single_image(
                data, name=name, plot=plot, verbose=verbose, print_output=print_output, **frame_params
            )
            rows.append(result)
            ...
            rows.append({col: (name if col == "name" else np.nan) for col in RESULT_COLUMNS})
...
def main(argv=None):
    ...
    for frame in frames:
        try:
            ...
            rows.append({col: (str(frame) if col == "filename" else "nan") for col in CSV_COLUMNS})
    ...
    if (len(frames) > 1 or failures) and not args.quiet:
        print(
            f"measured {len(rows) - len(failures)}/{len(frames)} frames" + (f", {len(failures)} failed" if failures else "")
        )
