...
class PHD2Connector(GuiderInterface, ImagerInterface):
    ...
    def _handle_event(self, ev):  # noqa: C901
        ...
        match e:
            case "AppState":
                ...
                boxed_debug(lines=[f"{function_name()}: {e}, app_state: {self.app_state}"], logger=logger)
            case "Alert":
                boxed_debug(lines=[f"{function_name()}: {e}, Type: {ev['Type']}, Msg: {ev['Msg']}"], logger=logger)
            case "Version":
                with self.lock:
            ...
            case "LockPositionLost":
                boxed_debug(logger=logger, lines=[f"{function_name()}: event: LockPositionLost"])
            ...
            case "SingleFrameComplete":
                ...
                boxed_debug(lines=[f"{function_name()}: SingleFrameComplete, {result=}"], logger=logger)
    ...
    def set_point(self) -> float | None:
        ...
        that happened to carry one: on 2026-09-08 it kept reading 5.0 through
        eighty minutes of guiding at 16.5 degC after a camera reconnect silently
        dropped the cooler, so the alarm built for exactly that episode fired on a
        stale number that happened to be correct (#109, #245).
        """
        try:
