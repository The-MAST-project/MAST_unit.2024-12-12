...
class TestSetLimitFrameRpc:
    ...
    These used to assert `need_to_reset_limit_frame` as well. That flag was armed
    here and consumed in the exposure path, which is how a guide loop's own crop
    came to be cleared by an unrelated exposure; it is gone, and so is the reset
    it armed (#245).
    """
