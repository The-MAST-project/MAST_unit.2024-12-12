...
class TestAcceptedIsNotApplied:
    ...
    def test_restoring_means_reading_first(self):
        ...
        c.set_limit_frame(roi=ImagerRoi.verbatim(x=0, y=0, width=8288, height=5644))
        fake.apply()
...
class TestTheCoolerSetPoint:
    """#109: a set point that was an echo of the last reply that happened to carry one."""

    def test_it_is_read_while_the_cooler_is_on(self):
        assert connector(FakePhd2(cooler_on=True, setpoint=5.0)).set_point == 5.0

    def test_a_cooler_that_is_off_reports_no_set_point(self):
        """PHD2 sends `setpoint` only inside `if (on)`. None is the truthful answer."""
        assert connector(FakePhd2(cooler_on=False, setpoint=5.0)).set_point is None

    def test_it_does_not_survive_the_cooler_going_off(self):
        """The 2026-09-08 episode: 80 minutes at 16.5 C still reporting a 5.0 set point."""
        fake = FakePhd2(cooler_on=True, setpoint=5.0)
        c = connector(fake)
        assert c.set_point == 5.0
        fake.cooler_on = False
        assert c.set_point is None, "a stale set point is what silenced the alarm built for this"


class TestNothingIsRemembered:
    """The structural check: no attribute on the connector shadows PHD2's own state."""

    @pytest.mark.parametrize(
        "attribute",
        ["limit_frame_in_force", "limit_frame_to_restore", "need_to_reset_limit_frame", "_setpoint"],
    )
    def test_the_caches_are_gone(self, attribute):
        assert not hasattr(PHD2Connector, attribute)
        assert attribute not in getattr(PHD2Connector, "__annotations__", {})
