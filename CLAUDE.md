...
**Update it in the same change; nothing checks it.** The test that asserted the guide named every check module and every `Tier` was withdrawn — a test over prose fails for the wrong reasons. So adding a `Tier` or a completion form, or changing what any static check enforces, means editing the guide's corresponding table in the same commit.

The static checks themselves live in `tests/contract/` and run in CI with the rest of the suite, so a violation reddens the pull request that introduces it. When you add or remove a module there, update the guide's table in the same change — nothing compares them.
...
`ps3cli.exe --server` validates a catalog on startup and exits (code 2) if absent. The minimal catalog that satisfies boot is `<cat>\UC4\Index.UC4` (may be empty) plus non-empty `<cat>\Orca\Orca0025.orc`, `StarOrca0025.orc`, `DistOrca0025.orc`. Set machine-scope `PS3CLI_DIR` and `PS3CLI_CATALOG` env vars so the app finds `ps3cli.exe` and its catalog whichever account runs it. The unit process is started by hand under the VS Code debugger as `mast`; no MAST Windows service is registered on any unit, and provisioning deletes any it finds.
