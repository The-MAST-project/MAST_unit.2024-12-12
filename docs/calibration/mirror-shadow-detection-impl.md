...
Supersedes the earlier sketches `src/science/find_vertical_obstruction.py` (column sums → vertical only; since removed) and the `mask_linear_shadow` half of `src/tools/vigneting/vignetting.py` (argmin X-profile → vertical only).
